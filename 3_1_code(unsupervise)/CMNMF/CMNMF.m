function [CMNMF_result_cell] = CMNMF(parameter_cell)    
    ...
    get_files_parameter_cell = {file_num_need;dir;name_prefix;total_gene_num;...,
...
%     matrix_cell_validation  = {gene_idx;groundTruth_pathway_old;gene_idx_pathway;ppi_old;ppi_old_index;validation_groundTruth_str};
    ...
    matrix_cell_validation  = {gene_idx;groundTruth_pathway_new;gene_idx_pathway;ppi_new;ppi_new_index;validation_groundTruth_str};
    ...
    tic;
    [learned_matrix_cell,best_parameter_array,evaluation_result] = learn(input_parameter_cell, matrix_cell_train, matrix_cell_validation, ...,
    ...
    CMNMF_result_cell = {evaluation_test;learned_matrix_cell;best_parameter_array;evaluation_result;max_ites;alpha_set;beta_set};
    toc;
    %%%%%%%%%test%%%%%%%%%%%%%%
    
    if strcmp(test_groundTruth_str,'ppi') == 1
        result_file_name = [method_dir 'CMNMF_' species_acronym '_result_ppi_' datestr(now,30) '.mat' ];  
        save(result_file_name,'CMNMF_result_cell');
    elseif strcmp(test_groundTruth_str,'pathway') == 1
        result_file_name = [method_dir 'CMNMF_' species_acronym '_result_pathway_' datestr(now,30) '.mat' ];  
        save(result_file_name,'CMNMF_result_cell');
    end  
...
function [file_name_cell] = get_files(get_files_parameter_cell)
    ...
    elseif count < file_num_need
        ...
        for i = 1:file_num_need - count
            file_name_new = [dir_path '/initial_matrix_fixed_' datestr(now,30) '.mat' ];            
