function [L,W,H1,H2] = CMNMF_LF( MaxIter,A1,A2,W,H1,H2,M,alpha,beta,lambda1,lambda2)
%L为目标函数值
    D1 = diag(sum(M,2));
    D2 = diag(sum(M,1));
 %L=zeros(1+2*Inner_MaxIter,MaxIter);
 L=zeros(MaxIter*3,4);
 [L(1,1),L(1,2),L(1,3),L(1,4)]=objective(alpha,beta,lambda1,lambda2,A1,A2,W,H1,H2,M);
for i=1:MaxIter  
    %update W 
    W=W.*(A1*H1'+alpha*A2*H2')./(W*(H1*H1')+alpha*W*(H2*H2')+lambda1*W+eps);
    t = (i-1)*3+2;
    [L(t,1),L(t,2),L(t,3),L(t,4)]=objective(alpha,beta,lambda1,lambda2,A1,A2,W,H1,H2,M);
    if(L(t,1) > L(t-1,1))
      break;
    end
    W(isnan(W))=0;    
