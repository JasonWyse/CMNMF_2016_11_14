function [ AHC_result ] = AHC2(species_acronym,file_cell,parameter_cell)
    ...
    Idx = clusterdata(g_p_network,'linkage','ward',K); 
    G_predict = zeros(m,K);
    for i = 1:m
        G_predict(i,Idx(i))=1; 
    end
    ...
    if strcmp(test_groundTruth_str,'pathway') ==1
        ...
        AHC_result = [F,Jaccard,RD,Precision,Recall,M_sim];
        AHC_result_cell = {G_predict;AHC_result};
        
        result_file_name = ['AHC_filled_' species_acronym '_result_pathway_' datestr(now,30) '.mat' ];  
        save([method_dir result_file_name],'AHC_result_cell');
    end
    if strcmp(test_groundTruth_str,'ppi') ==1
        ...
        result_file_name = ['AHC_filled_' species_acronym '_result_ppi_' datestr(now,30) '.mat' ];  
        save([method_dir result_file_name],'AHC_result_cell');
    end
