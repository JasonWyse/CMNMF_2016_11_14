function [ output_args ] = HNMF_ppi_difference( )
%HNMF_PPI_DIFFERENCE Summary of this function goes here
%   Detailed explanation goes here
    ...
    name_prefix = ['HNMF_' species_acronym '_result_ppi'];
    count =0;
    for i =1:length(fileNames)
       if(strncmp(fileNames(i),name_prefix,19) == 1)
           count = count+1;
           file_name_cell{count,1} = fileNames{i,1};
       end
    end
    load(file_name_cell{1,1},'HNMF_result_cell');
    learned_matrix_cell = HNMF_result_cell{2,1};
    best_parameter_array = HNMF_result_cell{3,1};
    ...
    HNMF_result_cell = {evaluation_test};
    %%%%%%%% save result %%%%%%%%
    result_file_name = ['HNMF_' species_acronym '_result_ppi_difference_' datestr(now,30) '.mat' ];  
    save(result_file_name,'HNMF_result_cell');
    %%%%%%%% save result %%%%%%%%
    rmpath(data_dir);
    rmpath(tool_dir);
